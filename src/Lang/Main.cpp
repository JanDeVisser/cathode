...
struct Builder {
    ...
    bool parse()
    {
        ...
        auto program = Lang::parse<Lang::Program>(parser, *std_lib, program_name);
        if (!program) {
        ...
        if (auto source_text_maybe = std::visit(
            ...
            auto mod = Lang::parse<Lang::Module>(parser, source_text, as_utf8(program_name));
    ...
    bool normalize()
    {
        auto normalized = Lang::normalize(parser.program);
        if (!parser.errors.empty()) {
